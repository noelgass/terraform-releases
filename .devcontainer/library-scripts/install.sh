...
find_version_from_terraform_releases() {
    ...
    if [ "$(echo "${requested_version}" | grep -o "." | wc -l)" != "2" ]; then
        local regex="terraform_\\K[0-9]+.[0-9]+.[0-9]+-[a-zA-Z0-9_]*"
        local version_list="$(curl -s https://releases.hashicorp.com/terraform/ | grep -oP "${regex}" | sort -rV)"
    ...
    if [ -z "${!variable_name}" ] || ! echo "${version_list}" | grep "^${!variable_name//./\\.}$" > /dev/null 2>&1; then
        echo -e "Invalid Terraform version: ${requested_version}\nValid values:\n${version_list}" >&2
        exit 1
    fi
...
find_version_from_terraform_releases TERRAFORM_VERSION
